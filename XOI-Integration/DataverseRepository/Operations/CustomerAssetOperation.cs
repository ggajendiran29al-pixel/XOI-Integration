...
namespace XOI_Integration.DataverseRepository.Operations
{
    public class CustomerAssetOperation
    {
        ...
        public static async Task CreateCustomerAssetAsync(ILogger _log, List<CustomerAssetToCreate> customerAssetsData, string customerName, string jobId)
        {
           ...
           //updated gg 05022026
            var customerId = await GetCustomerGuidAsync(_log, customerName, jobId);
                            ...
                            Filters =
{
    ...
    new FilterExpression(LogicalOperator.Or)
    {
        ...
        //gg updated code //updated on 05022026
       public static async Task<Guid> GetCustomerGuidAsync(ILogger _log, string customerName, string jobId = null)
{
    // 1) Prefer resolving Account GUID from Booking -> Work Order -> Service Account
    if (!string.IsNullOrWhiteSpace(jobId))
    {
        try
        {
            ...
            if (bookingIds != null && bookingIds.Any())
            {
                ...
                var woRef = booking?.GetAttributeValue<EntityReference>("msdyn_workorder");
                if (woRef != null)
                {
                    var wo = await DataverseApi.Instance.RetrieveAsync(
                        "msdyn_workorder",
                        woRef.Id,
                        new ColumnSet("msdyn_serviceaccount") // adjust if your org uses a different field
                    );

                    var acctRef = wo?.GetAttributeValue<EntityReference>("msdyn_serviceaccount");
                    if (acctRef != null && acctRef.Id != Guid.Empty)
                    {
                        _log.LogInformation($"[ACCOUNT] Resolved account from WO service account: {acctRef.Id}");
                        return acctRef.Id;
                    }

                    _log.LogWarning($"[ACCOUNT] WorkOrder {woRef.Id} has no msdyn_serviceaccount. Falling back to name lookup.");
                }
                else
                {
                    _log.LogWarning($"[ACCOUNT] Booking {bookingIds.First()} has no msdyn_workorder. Falling back to name lookup.");
                }
            }
            else
            {
                _log.LogWarning($"[ACCOUNT] No booking IDs found for JobId={jobId}. Falling back to name lookup.");
            }
        }
        catch (Exception ex)
        {
            _log.LogWarning($"[ACCOUNT] Failed WO-based account resolution. Falling back to name lookup. Error: {ex.Message}");
        }
    }

    // 2) Fallback: name-based lookup (ambiguous if duplicates)
    _log.LogInformation($"[ACCOUNT] Name-based lookup for customer '{customerName}'");
    ...
    var query = new QueryExpression("account")
    {
        ...
        TopCount = 2 // detect duplicates
    };

    var response = await DataverseApi.Instance.RetrieveMultipleAsync(query);

    if (response.Entities == null || !response.Entities.Any())
        throw new Exception($"Account not found by name: '{customerName}'");

    if (response.Entities.Count > 1)
        _log.LogWarning($"[ACCOUNT] Multiple Accounts found with name '{customerName}'. Using first result. This is risky.");

    var customerId = response.Entities.First().Id;

    _log.LogInformation($"[ACCOUNT] Name-based resolved account: {customerId}");
    return customerId;
}
