...
namespace XOI_Integration.DataverseRepository
{
    public class CustomerAssetDataHandler
    {
            ...
            string jobId)
        {
            ...
            //Updated gg 05022026
            var customerId = await CustomerAssetOperation.GetCustomerGuidAsync(_log, xOiJobInfo.CustomerName, jobId);
            ...
            foreach (var asset in customerAssetData)
            {
                ...
                var bookingIds2 = await BookableResourceBookingOperation.GetBookableResourceBookingIdsAsync(jobId);
                await CustomerAssetOperation.AssociateAssetToWorkOrderIncidentAsync(_log, assetId, bookingIds2);
            }
